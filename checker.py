...
if h1_tags is None or len(h1_tags) == 0:
    ...
    sys.exit(1)
print("h1 tag check passed...")
if not re.search(r".*States.*", h1_tags[0]):
    print("Title `States` doesn't found")
    sys.exit(1)
print("`States` title check passed...")
## LI state ID
li_tags = list(filter(None, [x.replace(" ", "").strip(" ").strip("\n").strip("\t") for x in tree.xpath('//body/ul/li/text()')]))
if li_tags is None or len(li_tags) != 13:
    print("Doesn't find 13 LI tags (found {})".format(len(li_tags)))
    sys.exit(1)
print("13 LI tags check passed ...")
...
if li_tags_b is None or len(li_tags_b) != 13:
    print("Doesn't find 13 LI tags with B tag (found {})".format(len(li_tags_b)))
    sys.exit(1)
